...
dense_rank() over (partition by year order by sum_paid desc) as rank
from t_1)
select t.*, r.name, r.surname, t.year-extract(year from (r.birth_date)) as age
from t_2 t
join riders r
...
| sum_apid | year |  customer_id | name | surname | age|
| :--- |     :---: |  ---: |  ---: |  ---: |  ---: |  
234.51	|2014	|17404	|Kent	|Pitts|33
237.51	|2015	| 38365	|Yvonne |Griffin|45
236.64|	2016|	43420	|Tracy|Burns|19
238.96	|2017|	30035	|Sharon	|Smith|20
240.85	|2018	|24188	|Charlotte|Winters|23
...
The below chart illustrates, the expected average of purchases from four customer groups over the next six months. From the chart, group A is the most valuable customer because the gap between the aforementioned group and groups B and C is noticeable. It is expected that each customer in group A on average spend 522 $ over the next six months. However, customers in group D on average only spend 15 $. **Bussiness recommendation: the marketing team should try to reactivate customers group D by sending promotions and discounts. Also, they should try to intrigue customers in groups B and C to increase their app usage. Finally, they should monitor customers in group A, and pay attention to any change in their purchase pattern.** Let's dig into the groups, and draw insights about their different behavior.
![payment](return-1.png)
**Bussiness insight: The below chart highlights the importance of the number of purchases. Indeed, group A as the most valuable group for us has a noticeably higher number of purchases compared to the other group. Thus, using marketing campaigns to increase the number of purchases is necessary. Another interesting point is that the difference between groups C and D is not high. This shows the difference between the behavior of groups C and D. In fact, customers in Group C spent more money on their trips or they had longer trips compared to group D.** 
![payment](num-1.png)
